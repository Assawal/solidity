...
std::map<YulString, BuiltinFunctionForEVM> createBuiltins(langutil::EVMVersion _evmVersion, bool _objectAccess)
{
	...
	if (_objectAccess)
	{
		builtins.emplace(createFunction("linkersymbol", 1, 1, SideEffects{}, {LiteralKind::String}, [](
			...
			_assembly.appendLinkerSymbol(literalToString(std::get<Literal>(arg)));
		}));
		...
		builtins.emplace(createFunction("datasize", 1, 1, SideEffects{}, {LiteralKind::String}, [](
			...
			auto const dataName = YulString(literalToString(lit));
		...
		builtins.emplace(createFunction("dataoffset", 1, 1, SideEffects{}, {LiteralKind::String}, [](
			...
			yulAssert(_call.arguments.size() == 1, "");
			Expression const& arg = _call.arguments.front();
			auto const dataName = YulString(literalToString(std::get<Literal>(arg)));
			if (_context.currentObject->name == dataName)
		...
		builtins.emplace(createFunction(
			...
			[](
				...
				auto const identifier = literalToString(std::get<Literal>(_call.arguments[1]));
		...
		builtins.emplace(createFunction(
			...
			[](
				...
				yulAssert(_call.arguments.size() == 1, "");
				_assembly.appendImmutable(literalToString(std::get<Literal>(_call.arguments.front())));
			}
		));
	}
	return builtins;
	...
	if (!function)
	{
		BuiltinFunctionForEVM builtinFunction = createFunction(
			...
			[=](
				...
				auto const& lit = std::get<Literal>(bytecode);
				_assembly.appendVerbatim(
					asBytes(literalToString(lit)),
					_arguments,
					_returnVariables
				);
			}
