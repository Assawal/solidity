...
DataFlowAnalyzer::DataFlowAnalyzer(
	...
	m_knowledgeBase([this](YulName _var) { return variableValue(_var); }, _yulNameRepository),
	m_analyzeStores(_analyzeStores == MemoryAndStorage::Analyze)
{
	if (m_analyzeStores)
	{
		if (auto const* builtin = _yulNameRepository.memoryStoreFunction(YulNameRepository::emptyName()))
			m_storeFunctionName[static_cast<unsigned>(StoreLoadLocation::Memory)] = builtin->name;
		if (auto const* builtin = _yulNameRepository.memoryLoadFunction(YulNameRepository::emptyName()))
			m_loadFunctionName[static_cast<unsigned>(StoreLoadLocation::Memory)] = builtin->name;
		if (auto const* builtin = _yulNameRepository.storageStoreFunction(YulNameRepository::emptyName()))
			m_storeFunctionName[static_cast<unsigned>(StoreLoadLocation::Storage)] = builtin->name;
		if (auto const* builtin = _yulNameRepository.storageLoadFunction(YulNameRepository::emptyName()))
...
void DataFlowAnalyzer::handleAssignment(std::set<YulName> const& _variables, Expression* _value, bool _isDeclaration)
{
	...
	MovableChecker movableChecker{m_yulNameRepository, &m_functionSideEffects};
	if (_value)
...
void DataFlowAnalyzer::clearKnowledgeIfInvalidated(Block const& _block)
{
	if (!m_analyzeStores)
		return;
	SideEffectsCollector sideEffects(m_yulNameRepository, _block, &m_functionSideEffects);
...
void DataFlowAnalyzer::clearKnowledgeIfInvalidated(Expression const& _expr)
{
	if (!m_analyzeStores)
		return;
	SideEffectsCollector sideEffects(m_yulNameRepository, _expr, &m_functionSideEffects);
	if (sideEffects.invalidatesStorage())
		m_state.environment.storage.clear();
	if (sideEffects.invalidatesMemory())
	{
...
{
	if (FunctionCall const* funCall = std::get_if<FunctionCall>(&_expression))
		if (funCall->functionName.name == m_yulNameRepository.hashFunction({}))
