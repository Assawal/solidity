...
{
public:
	explicit RematCandidateSelector(YulNameRepository const& _yulNameRepository): DataFlowAnalyzer(_yulNameRepository, MemoryAndStorage::Ignore) {}
	...
	void operator()(VariableDeclaration& _varDecl) override
	{
		...
		if (_varDecl.variables.size() == 1)
		{
			...
			if (AssignedValue const* value = variableValue(varName))
			{
				...
				m_expressionCodeCost[varName] = CodeCost::codeCost(m_yulNameRepository, *value->value);
			}
		}
	}
...
{
	RematCandidateSelector selector{_yulNameRepository};
	selector(_ast);
	...
	Rematerialiser::run(_yulNameRepository, _ast, std::move(varsToEliminate));
	// Do not remove functions.
	std::set<YulName> allFunctions = NameCollector{_ast, NameCollector::OnlyFunctions}.names();
	UnusedPruner::runUntilStabilised(_yulNameRepository, _ast, _allowMSizeOptimization, nullptr, allFunctions);
}
...
)
{
	...
	Rematerialiser::run(_yulNameRepository, _ast, std::move(varsToEliminate), true);
	// Do not remove functions.
	std::set<YulName> allFunctions = NameCollector{_ast, NameCollector::OnlyFunctions}.names();
	UnusedPruner::runUntilStabilised(_yulNameRepository, _ast, _allowMSizeOptimization, nullptr, allFunctions);
}
